#!/usr/bin/env python3
"""Deterministic supply-chain invariants gate for the registration-gate-cli release pipeline.
...
RELEASE_WF = os.path.join(WF_DIR, "registration-gate-cli-release.yml")
BUILD_CONTAINER_WF = os.path.join(WF_DIR, "registration-gate-cli-build-container.yml")
...
def check_gate_build_image(failures, text, require_real_digest):
    m = re.search(
        r"image:\s*"
        r"ghcr\.io/lee-mcfaul2/registration-gate-cli-build@sha256:(\S+)",
        text,
    )
    if not m:
        failures.append(
            "registration-gate-cli-release.yml: the build job's container image must be "
            "ghcr.io/lee-mcfaul2/registration-gate-cli-build@sha256:<digest>"
        )
        return
    digest = m.group(1)
    ...
    if require_real_digest:
        if digest == PLACEHOLDER_DIGEST:
            failures.append(
                "registration-gate-cli-release.yml: the build container image still uses "
                "the placeholder %r; --require-real-digest demands a real "
                "@sha256:<64-hex> digest (resolved by Task 9)"
                % PLACEHOLDER_DIGEST
            )
        elif not real:
            failures.append(
                "registration-gate-cli-release.yml: the build container image digest %r "
...
def check_build_job_block(failures, text):
    ...
    m = re.search(r"\n  build:\n", text)
    if not m:
        failures.append("registration-gate-cli-release.yml: missing 'build:' job")
        return
    start = m.start()
    nxt = re.search(r"\n  [A-Za-z][\w-]*:\n", text[start + 1:])
    ...
    if "contents: read" not in block:
        failures.append(
            "registration-gate-cli-release.yml: build job must declare 'contents: read'"
        )
    if "# NO id-token" not in block:
        failures.append(
            "registration-gate-cli-release.yml: build job must carry the "
            "'# NO id-token' marker"
        )
    if not re.search(r"matrix:\s*\{\s*instance:\s*\[1,\s*2\]\s*\}", block) \
            and not re.search(
                r"instance:\s*\n(?:\s*-\s*1\s*\n\s*-\s*2)", block):
        failures.append(
            "registration-gate-cli-release.yml: build job must use matrix "
            "instance: [1, 2]"
        )
    ...
    if "id-token: write" in block:
        failures.append(
            "registration-gate-cli-release.yml: build job must NOT grant "
...
def check_attest_job_permissions(failures, text):
    ...
    if not m:
        failures.append(
            "registration-gate-cli-release.yml: missing 'attest-sign-publish:' job"
        )
        return
    start = m.start()
    nxt = re.search(r"\n  [A-Za-z][\w-]*:\n", text[start + 1:])
    ...
    if "id-token: write" not in block:
        failures.append(
            "registration-gate-cli-release.yml: attest-sign-publish must have "
            "'id-token: write'"
        )
...
def check_simple_substrings(failures, release_text):
    if "refs/tags/registration-gate-cli-v*" not in release_text:
        failures.append(
            "registration-gate-cli-release.yml: missing tag-ref guard "
            "'refs/tags/registration-gate-cli-v*'"
        )
    if "COSIGN_VERSION: v3.0.6" not in release_text:
        failures.append(
            "registration-gate-cli-release.yml: COSIGN_VERSION must be pinned to v3.0.6"
        )
    if "cosign checksum mismatch" not in release_text:
        failures.append(
            "registration-gate-cli-release.yml: missing fail-closed "
...
def check_provenance_sha(failures, release_text, build_text):
    ...
    for fname, text in (
        ("registration-gate-cli-release.yml", release_text),
        ("registration-gate-cli-build-container.yml", build_text),
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Supply-chain invariants gate for registration-gate-cli workflows."
    )
    parser.add_argument(
        "--require-real-digest",
        action="store_true",
    ...
    check_uses_pinned(failures, "registration-gate-cli-release.yml", release_text)
    check_uses_pinned(failures, "registration-gate-cli-build-container.yml", build_text)

    check_no_pipe_to_shell(failures, "registration-gate-cli-release.yml", release_text)
    check_no_pipe_to_shell(
        failures, "registration-gate-cli-build-container.yml", build_text
    )

    check_gate_build_image(failures, release_text, args.require_real_digest)
    ...
    print(
        ...
        "registration-gate-cli-release.yml and registration-gate-cli-build-container.yml"
